...
系统实现了完整的实验数据记录功能，支持将实验结果同时输出到控制台和Excel文件：
...
2. **数据组织结构**
   ```
   experiment_results/
   └── experiment_results_[timestamp].xlsx
       ├── 有效性实验
       │   ├── 算法类型
       │   ├── 总成本
       │   ├── 总时间(分钟)
...
5. **使用方法**
   ...
   result.put("算法类型", "GA");
   result.put("总成本", 123.45);
   result.put("总时间", 98.76);
...
7. **输出示例**
   ...
   === 效率实验 ===
   规模 50:
   GA: 耗时 234 ms, 成本 287.65
   
   === 稳定性实验 ===
   使用 GA:
   成本: 平均值=198.45, 方差=12.34
   时间: 平均值=112.89, 方差=5.67
   ```
